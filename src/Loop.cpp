...
//
// $Source: /home/pablo/Desarrollo/sags-cvs/clienttext/src/Loop.cpp,v $
// $Revision: 1.1 $
...
void CatchSignal (int sig)
{
	...
	{
	case SIGTERM:
		Logs.Add (Log::Notice, _("CatchSignal: Received SIGTERM"));
		break;
	case SIGINT:
		Logs.Add (Log::Notice, _("CatchSignal: Received SIGINT"));
		break;
	case SIGPIPE:
		Logs.Add (Log::Notice, _("CatchSignal: Received SIGPIPE"));
			return;
	case SIGHUP:
		Logs.Add (Log::Notice, _("CatchSignal: Received SIGHUP"));
		break;
	default:
		;
	}
...
int SelectLoop::Run (void)
{
	...
	while (1)
	{
		...
		if (select_output < 0 && errno == EINTR)
		{
			...
			Logs.Add (Log::Debug, _("pselect was interrupted"));
			continue;
		}
...
void SelectLoop::AddTimeout (int seconds)
{
	...
	Logs.Add (Log::Debug, _("Timeout seted to %d seconds"), seconds);
}
...
void SelectLoop::DeleteTimeout (void)
{
	if (timeout != NULL)
	{
		delete timeout;
		timeout = NULL;
		Logs.Add (Log::Debug, _("Timeout removed"));
	}
}
